...
class Userbot(Client):
    ...
    async def start(self):
        ...
        if config.STRING1:
            ...
            try:
                await self.one.send_message(config.LOGGER_ID, f"**» Assistent başladı :**\n\n✨ ɪᴅ : `{self.one.id}`\n❄ ɴᴀᴍᴇ : {self.one.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.one.username}"
                )
            except:
        ...
        if config.STRING2:
            ...
            try:
                await self.two.send_message(config.LOGGER_ID, f"**» Assistent başladı :**\n\n✨ ɪᴅ : `{self.two.id}`\n❄ ɴᴀᴍᴇ : {self.two.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.two.username}"
                )
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING3:
            ...
            try:
                await self.three.send_message(config.LOGGER_ID, f"**» Assistent başladı :**\n\n✨ ɪᴅ : `{self.three.id}`\n❄ ɴᴀᴍᴇ : {self.three.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.three.username}"
                )
            except:
                LOGGER(__name__).error(
